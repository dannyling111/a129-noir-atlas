...
with sync_playwright() as p:
    ...
    try:
        ...
        js(mobile,'Mobile closeup yields compact 11px bubble beside the head', '''()=>{const a=noirApp;a.cameraRig.frameActor();a.setPaused(true);a.dialogue.say(a.actor,'镜头可以拉近，文字不会跟着缩小。',20);a.time+=3;a.draw();const b=a.dialogue.layouts[0],head=a.screen(a.actor.headWorld),foot=a.screen([a.actor.x,a.actor.y,a.actor.z]);const el=document.querySelector('.dialogue-bubble');const tray=document.getElementById('view-tray');const director=document.getElementById('director-toggle');return {ok:!!b&&b.x>=0&&b.x+b.w<=innerWidth&&b.y>=0&&b.y+b.h<innerHeight-150&&b.font===11&&b.w<=160&&Math.abs(head.y-foot.y)>100&&getComputedStyle(director).display===\'none\'&&tray.hidden&&el.getBoundingClientRect().height<90,bodyPixels:Math.abs(head.y-foot.y),bubble:b};}''')
